...
type DiskPartition struct {
	...
	IoTime       uint64  `json:"ioTime"`       // IO 时间
}
...
func updateDiskIOStats(disks []DiskPartition, deviceMap map[string]string) error {
	...
	for i := range disks {
		...
		for _, c := range counters {
			if c.Name == baseDevice || c.Name == disks[i].Device {
				counter = &c
				break
			}
		}
		...
		// 保存当前统计
		currentStats[counter.Name] = DiskIOHistory{
		...
		// 计算速度（需要上次的数据）
		if lastStat, ok := lastStats[counter.Name]; ok && duration > 0 {
			readSpeed, writeSpeed := CalculateDiskSpeed(currentStats[counter.Name], lastStat, duration)
