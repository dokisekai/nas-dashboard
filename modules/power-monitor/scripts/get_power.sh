...
# =================================================================
# 硬件功耗全真实监测工具 (v1.6.1) - 100% 硬件审计修正版
# =================================================================
...
echo -e "${BLUE}=====================================================${NC}"
echo -e "${BLUE}          服务器硬件 100% 真实审计及功耗报告         ${NC}"
echo -e "${BLUE}=====================================================${NC}"

# 1. 核心硬件审计清单
CPU_MODEL="Intel i7-13700K (16C/24T)"
GPU_MODEL="XFX Speedster MERC319 RX 6950 XT (16GB)"
...
sleep 1
read E2 < "$RAPL_PKG"
CPU_W=$(echo "scale=2; ($E2 - $E1) / 1000000" | bc)
...
# 5. 电源效率转换损耗
RAW_TOTAL=$(echo "$CPU_W + $GPU_W + $HDD_W + $SSD_ALL_W + $MB_RAM_W + $COOL_W + $USB_W" | bc)
...
TOTAL_MIN=$(echo "$CPU_W + $GPU_W + $HDD_MIN + $SSD_ALL_MIN + $MB_RAM_MIN + $COOL_MIN + 2.0 + $LOSS_MIN" | bc)
TOTAL_MAX=$(echo "$CPU_W + $GPU_W + $HDD_MAX + $SSD_ALL_MAX + $MB_RAM_MAX + $COOL_MAX + 3.0 + $LOSS_MAX" | bc)

# 6. 输出报告
echo -e "监测时间: $(date '+%Y-%m-%d %H:%M:%S')"
echo -e "-----------------------------------------------------"
echo -e "${GREEN}[传感器测量]${NC}"
printf "  %-26s %10s W\n" "CPU (13700K RAPL):" "$CPU_W"
printf "  %-26s %10s W\n" "GPU (6950 XT SoC):" "$GPU_W"

echo -e "${YELLOW}[状态监测推算]${NC}"
printf "  %-26s %10s W (休眠中)\n" "HDD 机械硬盘 (2块):" "$HDD_W"
printf "  %-26s %10s W (待机中)\n" "SSD 固态硬盘 (3块):" "$SSD_ALL_W"

echo -e "${PURPLE}[硬件规格预估]${NC}"
printf "  %-26s %10s W\n" "主板 & 2x32G内存:" "$MB_RAM_W"
printf "  %-26s %10s W\n" "散热 (水冷泵+风扇):" "$COOL_W"
printf "  %-26s %10s W\n" "USB及外设功耗:" "$USB_W"
printf "  %-26s %10s W\n" "1100W金牌损耗 (估):" "$LOSS_W"
